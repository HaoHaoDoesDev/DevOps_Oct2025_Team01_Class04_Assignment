describe("End-to-End User Lifecycle Flow", () => {
  ...
  it("User uploads a file", () => {
    ...
    cy.get('input[type="file"]').selectFile("cypress/fixtures/testfile.txt", {
      force: true,
    });
  ...
  it("User downloads their file", () => {
    ...
    cy.contains("testfile.txt").closest("div.group").as("firstImage");
  ...
  it("User deletes an image", () => {
    ...
    cy.get("@firstImage").find('button[aria-label="Delete image"]').click();
  ...
  it("Admin deletes the created user", () => {
    ...
    cy.contains("td", userEmail).parent("tr").as("userRow");

    cy.get("@userRow")
      .find('button[data-slot="dropdown-menu-trigger"]')
      .click();

    cy.contains('div[role="menuitem"]', "Delete User").click();
