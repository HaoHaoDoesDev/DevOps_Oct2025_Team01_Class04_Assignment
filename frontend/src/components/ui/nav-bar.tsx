...
const CardNav: React.FC<CardNavProps> = ({
  ...
  return (
    <div className={`card-nav-container absolute left-1/2 -translate-x-1/2 w-[90%] max-w-200 z-99 top-[1.2em] md:top-[2em] ${className}`}>
      <nav
        ref={navRef}
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-15 flex items-center justify-between p-2 pl-[1.1rem] z-2">
          ...
          >
            <div className={`hamburger-line w-7.5 h-0.5 bg-current transition-all duration-300 ${isHamburgerOpen ? 'translate-y-1 rotate-45' : ''}`} />
            <div className={`hamburger-line w-7.5 h-0.5 bg-current transition-all duration-300 ${isHamburgerOpen ? '-translate-y-1 -rotate-45' : ''}`} />
          </div>

          <div className="logo-container flex items-center md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2">
            <Image src="/assets/logo/devops-assignment-logo.png" alt="Logo" width={150} height={40} className="cursor-pointer" />
          </div>
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            ...
            >
              <div className="font-normal text-[18px] md:text-[22px] tracking-tight">{item.label}</div>
